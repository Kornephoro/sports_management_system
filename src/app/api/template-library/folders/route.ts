...
import {
  createTemplateLibraryFolderUseCase,
  listTemplateLibraryFoldersUseCase,
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listTemplateLibraryFoldersUseCase({ userId });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await createTemplateLibraryFolderUseCase(body);
