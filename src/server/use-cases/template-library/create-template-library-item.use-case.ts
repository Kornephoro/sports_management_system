...
const CreateTemplateLibraryItemInputSchema = z.object({
  userId: UuidLikeSchema,
  name: z.string().trim().min(1).max(120),
  description: z.string().optional(),
  splitType: z.string().trim().min(1).max(48).default("custom"),
  folderKey: z.string().trim().min(1).max(64).optional().nullable(),
  aliases: z.array(z.string().trim().min(1)).default([]),
  enabled: z.boolean().default(true),
  notes: z.string().optional(),
  units: z.array(TemplateLibraryUnitInputSchema).default([]),
});

export type CreateTemplateLibraryItemInput = z.input<typeof CreateTemplateLibraryItemInputSchema>;
...
export async function createTemplateLibraryItemUseCase(rawInput: CreateTemplateLibraryItemInput) {
  const input = CreateTemplateLibraryItemInputSchema.parse(rawInput);
        ...
        rpe_max: unit.rpeMax ?? null,
            ...
            superset_group: unit.supersetGroup
              ? {
                  ...
                  group_name: normalizeText(unit.supersetGroup.groupName ?? undefined),
  ...
  const created = await createTemplateLibraryItem({
    ...
    folder_key: input.folderKey ?? null,
  ...
  return toTemplateLibraryItemDto(created);
}
